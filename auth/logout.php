...
if (isset($_COOKIE[WEBSITE_NAME . "SESSION_ID"]) || isset($_COOKIE[WEBSITE_NAME . 'SESSION_NAME'])) {
	setcookie(WEBSITE_NAME . "SESSION_ID", false, 1, '/', WEBSITE_DOMAIN, true, true);
	setcookie(WEBSITE_NAME . "SESSION_NAME", false, 1, '/', WEBSITE_DOMAIN, true, true);
	
	$db->query("DELETE FROM auth_sessions WHERE session_id = '" . $db->real_escape_string($_COOKIE[WEBSITE_NAME . 'SESSION_ID']) . "' OR session_name = '" . $db->real_escape_string($_COOKIE[WEBSITE_NAME . 'SESSION_NAME']) . "' LIMIT 1");
}
